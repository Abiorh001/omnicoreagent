#!/usr/bin/env python3
"""
Agent with Database Memory Integration

This example shows how to build an agent that uses database memory
for persistent conversation history and state management.
"""
...
class AgentWithDatabaseMemory:
    """An agent that uses database memory for persistent conversations."""

    def __init__(self, db_url: str = "sqlite:///agent_memory.db"):
        ...
        self.agent_config = AgentConfig(
            agent_name="database_memory_agent",
        ...
        # Memory store for current session
        self.memory_store = InMemoryStore(max_context_tokens=30000)
    ...
    async def _load_conversation_history(self):
        ...
        logger.info(f"Loaded {len(self.current_session.events)} events into memory")
    ...
    async def process_message(self, user_message: str) -> str:
        ...
        try:
            ...
            # Add to memory
            await self.memory_store.store_message(
                "user", user_message, session_id=self.current_session.id
            )

            # Create agent response
            agent_response = await self._generate_agent_response(user_message)
            ...
            # Add to memory
            await self.memory_store.store_message(
                "assistant", agent_response, session_id=self.current_session.id
            )
    ...
    async def _generate_agent_response(self, user_message: str) -> str:
        ...
        try:
            ...
            message_history = await self.memory_store.get_messages(
                session_id=self.current_session.id
            )
            ...
            response = await react_agent._run(
                ...
                message_history=lambda: message_history,
                debug=True,
...
async def interactive_chat():
    ...
    try:
        ...
        session = await agent.start_conversation(
            app_name="interactive_chat", user_id="user_001"
        )

        print(f"🤖 Agent initialized with session: {session.id}")
...
async def main():
    ...
    print("Agent with Database Memory Examples")
    print("=" * 50)
