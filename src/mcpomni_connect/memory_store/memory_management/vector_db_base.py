...
class VectorDBBase(ABC):
    ...
    def __init__(self, collection_name: str, **kwargs):
        ...
        # Extract LLM connection if provided
        ...
        if not is_vector_db_enabled():
            ...
            if not is_embedding_requirements_met():
                ...
                # Use LLM connection for embeddings
                self._embed_model = None
                self._vector_size = None
                self.enabled = True
    ...
    def embed_text(self, text: str) -> List[float]:
        ...
        try:
            # First attempt: try to embed the full text
            ...
            # Check if response is None (could be due to rate limits)
            if response is None:
                logger.warning(
    ...
    def _embed_text_with_chunking(self, text: str) -> List[float]:
        ...
        try:
            ...
            # Combine all embeddings (simple average)
            if not all_embeddings:
                raise RuntimeError("All chunk embeddings failed")

            # Ensure all embeddings have the same dimension from config
    ...
    def _process_embedding_response(self, response) -> List[float]:
        ...
        # Convert base64 to float if needed (e.g., Voyage AI)
